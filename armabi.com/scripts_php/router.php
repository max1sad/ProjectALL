...
function getRequestPath() {
    ...
    global $file;
    $file->fileWrite($path);
    return '/' . ltrim(str_replace('/armabi.com/scripts_php/router.php', '', $path), '/');
    #}
}
...
function structurArm($ar_arm) {
    ...
    foreach($ar_arm as $row){
        ...
        $decsr = "<strong>".$row["Имя_хоста_(ip_и_mac_адрес)"]."<br>ip:".$row['ipAddr']."<br>"."mac: ".$row['macAddr']."</strong>";
        $new_arr['Имя_хоста_(ip_и_mac_адрес)'] = $decsr;
        if ($row['Регистрация'] == "t"){
            $new_arr['Регистрация'] = '<img src="images/Icons/check2.png" id="mn-1"></img>';
        } else {
            $new_arr['Регистрация'] = '<img src="images/Icons/fail.png" id="mn-1"></img>';
        }
        $new_arr['Имя_(роль)_в_домене'] = $row['Имя_(роль)_в_домене'].' ('.$row['description'].')';
        
        if ($row['Контр.домена'] == "1"){
            $new_arr['Контр.домена'] = '<img src="images/Icons/check2.png" id="mn-1"></img>';
        } else {
            $new_arr['Контр.домена'] = $row['Контр.домена'];
        }
        if ($row['Активность_хоста'] == "t"){
            $new_arr['Активность_хоста'] = '<img src="images/Icons/check2.png" id="mn-1"></img>';
        } else {
            $new_arr['Активность_хоста'] = '<img src="images/Icons/fail.png" id="mn-1"></img>';
        }
        if ($row['Активность_агента'] == "t"){
            $new_arr['Активность_агента'] = '<img src="images/Icons/check2.png" id="mn-1"></img>';
        } else {
            $new_arr['Активность_агента'] = '<img src="images/Icons/fail.png" id="mn-1"></img>';
        }
        $new_arr['id'] = $row['id'];
...
// функция для корня
function hello() {
    $sql = "select \"hostName\" as \"Имя хоста (ip/mac адрес)\",\"ipAddr\",\"macAddr\", \"isConfirm\",\"domainName\",description,\"isDomainContr\",\"isActiveHost\",\"isActiveAgent\" from devices  ;";
...
function getDevices() {
    ...
    $sql = "select \"hostName\" as \"Имя_хоста_(ip_и_mac_адрес)\",\"ipAddr\",\"macAddr\", \"isConfirm\" as \"Регистрация\",\"domainName\" as \"Имя_(роль)_в_домене\",description,\"isDomainContr\" as \"Контр.домена\",\"isActiveHost\" as \"Активность_хоста\",\"isActiveAgent\" as \"Активность_агента\",id,\"isActiveHost\",\"isActiveAgent\" from devices  ;";
    $result = sqlResult($sql);
#var_dump($result);

    $result = structurArm($result);
        $file->fileWrite(json_encode($result));
    return json_encode($result);
}
